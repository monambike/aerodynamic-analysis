...
read_airfoil_data <- function(df, airfoil_name) {
...
df_asa <- read_airfoil_data(aircraft_data$airfoil_wing, "Asa")
df_eh  <- read_airfoil_data(aircraft_data$airfoil_horizontal_stabilizer, "Emp. Horizontal")
df_ev  <- read_airfoil_data(aircraft_data$airfoil_vertical_stabilizer, "Emp. Vertical")
Resultados_Aviao_3d <- read_airfoil_data(aircraft_data$aircraft, "Aeronave")
...
# 
# plot_airfoil_wing <- function() {
#   df_asa_long <- df_2d %>%
#     filter(Perfil == "Asa") %>%
#     select(alpha, CL, CD, cl_cd) %>%
#     pivot_longer(
#       cols = c(CL, CD, cl_cd),
#       names_to = "Metric",
#       values_to = "Value"
#     )
#   
#   ggplot(df_asa_long, aes(alpha, Value)) +
#     geom_line() +
